...
export async function generateWelcomeCard(data) {
  ...
  try {
    ...
    try {
      if (data.isCustom) {
        const buffer = await downloadBinary(`guilds/${data.guildId}/assets/welcome_bg.png`)
        if (buffer) {
    ...
    ctx.beginPath()
    ctx.arc(centerX, centerY, radius, 0, Math.PI * 2)
    ctx.strokeStyle = data.cardAccentColor
    ctx.lineWidth = 4
    ctx.stroke()
    ...
    ctx.fillStyle = data.cardAccentColor
    ctx.fillText('WELCOME TO ' + data.guildName.toUpperCase(), 200, 70)
    ...
    ctx.fillText('Member #' + data.memberCount, 200, 148)

    // STEP 6 — Return buffer
    const buffer = await canvas.toBuffer('png')
    return buffer
  } catch (err) {
    console.error('[card] generateWelcomeCard failed:', err.message)
    throw err
  }
}
